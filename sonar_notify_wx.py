...
if __name__ == "__main__":
    ...
    #parser.add_argument("-l", "--list", help="获取所有group下的主机",action="store_true")
    #parser.add_argument("-u","--user", choices=('hitales','hitales-ops'),help="指定授权的用户,hitales/hitales-ops")
    parser.add_argument("-c", "--corpid", help="指定企业微信corpid")
    parser.add_argument("-s","--secret", help="指定企业微信应用secret")
    parser.add_argument("-a","--agentid", help="指定企业微信应用agentid")
    parser.add_argument("-u","--user", default="@all", help="成员ID列表（消息接收者，多个接收者用'|'分隔，最多支持1000个）。默认指定为@all，则向关注该企业应用的全部成员发送")
    parser.add_argument("-r","--reponame", help="指定仓库名称")
    parser.add_argument("-b", "--branch", help="指定代码分支")
    parser.add_argument("-v", "--version", help="指定编译版本")
    parser.add_argument("-i", "--sonarurl", help="指定sonarqube系统url地址")
    args = parser.parse_args()

    Token = GetToken(args.corpid, args.secret)
    Detail_url = '%s/dashboard?id=%s' %(args.sonarurl, args.reponame)
    status, bug, leak, code_smell, security, coverage, density = SonarqubeDetail(args.sonarurl, args.reponame)

    # 判断新代码质量阀状态
    title = ''
    description = ''
    if status == 'OK':
        title = "✅恭喜, 代码[%s]-分支[%s]#%s 扫描通过~👍" %(args.reponame, args.branch, args.version)
        description = """
...
""" %(bug, leak, code_smell, security, coverage, density)
        print "代码扫描成功！！！"
        res = SendCardMessage(Token, args.user, args.agentid, title, description, Detail_url)
        ...
        title = "❌真遗憾, 代码[%s]-分支[%s]#%s 扫描未通过~😭" %(args.reponame, args.branch, args.version)
        description = """
Bug数: <div class=\"highlight\">%s 个</div><br>
漏洞数: <div class=\"highlight\">%s 个</div><br>
可能存在问题代码行数: <div class=\"highlight\">%s 行</div><br>
